...
export async function ensureCrmSchema() {
  ...
  _crmReady = (async () => {
    ...
    try {
      ...
      // Sessões server-side (Fase 1). Idempotente.
      await s.unsafe(`
...
async function applyCampaignsSchema(db: ReturnType<typeof sql>): Promise<void> {
  await db.unsafe(`
    ...
    -- Cor de campanha (identificação visual na fila de respostas).
    ALTER TABLE public.campaigns ADD COLUMN IF NOT EXISTS color TEXT NOT NULL DEFAULT '#6B7280';

    -- Fila de respostas de campanha no atendimento.
    ALTER TABLE public.conversations ADD COLUMN IF NOT EXISTS campaign_service_status TEXT;
    ALTER TABLE public.conversations ADD COLUMN IF NOT EXISTS campaign_last_inbound_at TIMESTAMPTZ;
    ALTER TABLE public.conversations ADD COLUMN IF NOT EXISTS campaign_last_human_reply_at TIMESTAMPTZ;

    DO $$
    BEGIN
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'campaigns_color_hex_check'
      ) THEN
      ...
      END IF;
      IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'conversations_campaign_service_status_check'
      ) THEN
        ALTER TABLE public.conversations
          ...
          CHECK (
            ...
            OR campaign_service_status IN (
              'awaiting_reply', 'in_service', 'answered', 'completed',
